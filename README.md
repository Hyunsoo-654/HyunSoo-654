...
| **RISC-V AMBA APB** <br> 📅(25.08.26 ~ 25.08.29) | - RISC-V Multi-Cycle CPU와 저전력/저속 통신을 위한 AMBA APB (Advanced Peripheral Bus) 프로토콜을 활용 <br> - 주변장치(Peripheral)를 설계하고 검증 | 대한상공회의소 | [RISC-V / AMBA_APB](https://github.com/71105673/RISC-V/tree/main/AMBA_APB) |
|**AIX4_Lite Protocol** <br> 📅(25.09.04 ~ 25.09.05)| - 고속 데이터 통신을 위한 AMBA AXI4-Lite 프로토콜의 구조를 이해하고, 이를 기반으로 **FND(7-Segment) 컨트롤러 IP(지적 재산)**를 설계하는 것을 목표로 합니다. <br> - 최종적으로는 SystemVerilog로 설계된 IP를 검증하고, C언어를 통해 FPGA 보드에서 실제 동작을 구현 | 대한상공회의소 | [RISC-V / AXI4_Lite](https://github.com/71105673/RISC-V/tree/main/AXI4_Lite) |
| **CPU Design** <br> 📅(25.08.14 ~ 25.08.29) | - RISC-V 명령어 세트 아키텍처를 기반으로 구현한 32비트 **(Single, Multi)-Cycle** CPU 설계<br> - CPU의 핵심 구성 요소인 **DataPath**와 **ControlUnit**을 System Verilog를 사용하여 설계<br> - Vivado EDA tool 및 Synopsys VCS & Verdi를 활용하여 RTL 코드를 합성 후 시뮬레이션을 통해 **모든 명령어 타입 검증**.<br> - C언어 코드를 머신코드로 변환하여 실행함으로써, 소프트웨어와 하드웨어 간의 동작방식을 이해 | 대한상공회의소 | [RISC-V / RV32I-32bit CPU Design](https://github.com/71105673/RISC-V/tree/main/RV32I-32bit%20CPU%20Design) <br><br> [MultiCycle-Verification](https://github.com/71105673/RISC-V/tree/main/RV32I-32bit%20CPU%20Design/Multi%20Cycle%20RAM)|
...
| **2Stage 16bit MultiPlier** <br> 📅(24.05.30 ~ 25.06.20)  | - Cadence Virtuoso를 활용해 **Schematic → Layout** 전 과정을 수행 <br> - **Wallace Tree 구조** 기반으로 Multiplier 설계 및 검증 <br> - 2.2GHz 동작속도를 달성 | 인하대학교 <br> (정보통신공학과) | [2Stage-16bit-MultiPlier](https://github.com/71105673/2Stage-16bit-MultiPlier) |
| **근사 가산기를 이용한 저전력 디지털 신호 처리** <br> 📅(24.02.12 ~ 24.06.28)  | - Cadence Virtuoso 및 Python을 활용한 프로젝트 수행 <br> - 모바일 기기, IoT 확산 → 저전력 연산 중요성 증가 <br> - 이미지 프로세싱은 데이터량이 많아 전력 소모 큼 <br> - 정밀 가산기(FA)는 정확하지만 전력 소모 ↑ <br> - 근사 가산기(Approximate Adder) → 약간의 오류를 허용하고 전력/면적 효율 개선| 인하대학교 <br> (정보통신공학과) | [근사 가산기를 활용한 저전력 디지털 신호처리](https://github.com/71105673/Low-Power-Digital-Signal-Processing-Using-Approximate-Adders) |
